package com.checkin.facility.view;
...
public class FacilityClient {
	// mStorage simulates the backend database with an ArrayList
	private ArrayList<Facility> storage;
	public boolean PRINT = true;
	public FacilityClient(){
		storage = new ArrayList<Facility>();
	}
	// listFacilties: list facilities 
	public ArrayList<Facility> listFacilities(){
		if(PRINT){
			if(storage.size() == 0) System.out.println("There is no facilities at the moment");
	...
	public int requestAvailableCapacity(Facility f) {
		if(PRINT){
			System.out.println(f.getUnits().size());
		}
	...
	public ArrayList<Inspection> listInspections(Facility f){
		if(PRINT) {
			...
			if(f.getInspections().size() == 0){
				System.out.print("There is no inspections");
			}
	...
	public double calcMaintenanceCostForFacility(Facility f) {
		...
		if(PRINT) {
			System.out.println("It will cost " + rtn + " to finish all outstanding maintence for facility " + f.getId());
		}
		return rtn;
	}
